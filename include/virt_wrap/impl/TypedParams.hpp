...
namespace virt {
    ...
    TypedParameter::TypedParameter(const virTypedParameter &from) : TypedParameter(from,
                                                                                   no_name_tag{}) { name = from.field; }
    ...
    void TypedParams::add(const TypedParameter &tp) {
        ...
        std::visit(
                Visitor{// I hate when I can't just lift a member overload set
                        ...
                        [&, this](int v) { add(name, v); }, [&, this](unsigned v) { add(name, v); },
                        [&, this](long long v) { add(name, v); },
                        [&, this](unsigned long long v) { add(name, v); }, [&, this](double v) { add(name, v); },
    ...
    T TypedParams::get(gsl::czstring<> name) const {
        const auto it = std::find_if(underlying, underlying + size,
                                     [&](const virTypedParameter &tp) { return std::strcmp(name, tp.field) == 0; });
        return std::get<T>(TypedParameter{it, TypedParameter::no_name_tag{}}.val);
    }

    template<typename T>
    T &TypedParams::get(gsl::czstring<> name) {
        auto it = std::find_if(underlying, underlying + size,
                               [&](const virTypedParameter &tp) { return std::strcmp(name, tp.field) == 0; });
        return std::get<T>(TypedParameter{it, TypedParameter::no_name_tag{}}.val);
    }
}