...
namespace virt {
TypedParameter::TypedParameter(const virTypedParameter& from,
                               virt::TypedParameter::no_name_tag) {
    switch (from.type) {
...
TypedParameter::TypedParameter(const virTypedParameter& from)
    : TypedParameter(from, no_name_tag{}) {
    name = from.field;
}
...
template <typename T> T TypedParams::get(gsl::czstring<> name) const {
    const auto it = std::find_if(underlying, underlying + size,
                                 [&](const virTypedParameter& tp) {
                                     return std::strcmp(name, tp.field) == 0;
                                 });
