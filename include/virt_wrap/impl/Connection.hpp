...
namespace virt {
    ...
    ConnectionAuth<Callback>::operator virConnectAuth() noexcept {
        ...
        ret.cb = +[](virConnectCredentialPtr creds_ptr, unsigned int ncreds, void *cbdata) -> int {
            ...
            std::transform(creds_ptr, creds_ptr + ncreds, std::back_inserter(creds),
    ...
    void Connection::registerCloseCallback(void (*cb)(Data &), std::unique_ptr<Data> data) {
        virConnectRegisterCloseCallback(underlying, reinterpret_cast<virConnectCloseFunc>(cb), &data,
    ...
    auto Connection::listDefinedDomains<gsl::zstring<>>() const {
        ...
        std::transform(std::make_move_iterator(buf.begin()), std::make_move_iterator(buf.end()),
    ...
    auto Connection::getAllDomainStats(Domain::Stats::Types stats,
                                       Connection::GetAllDomains::Stats::Flags flags) -> std::vector<Domain::Stats::Record> {
        ...
        std::transform(*ptr, *ptr + res, std::back_inserter(recs),
                       [](const virDomainStatsRecord &rec) { return Domain::Stats::Record{rec}; });
    ...
    operator|(Connection::List::Domains::Flags lhs, Connection::List::Domains::Flags rhs) noexcept {
