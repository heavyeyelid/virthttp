...
ConnectCredential::ConnectCredential(const virConnectCredential& in) noexcept
    : type(CredentialType(in.type)), prompt(in.prompt), challenge(in.challenge),
      defresult(in.defresult) {}

template <typename Callback>
template <typename Container>
inline ConnectionAuth<Callback>::ConnectionAuth(Container c, Callback callback)
    : callback(callback) {
...
ConnectionAuth<Callback>::operator virConnectAuth() noexcept {
    ...
    ret.cb = +[](virConnectCredentialPtr creds_ptr, unsigned int ncreds,
                 void* cbdata) -> int {
        std::vector<ConnectCredential> creds{};
        creds.reserve(ncreds);
        std::transform(
            creds_ptr, creds_ptr + ncreds, std::back_inserter(creds),
            [](const auto& cred) { return ConnectCredential{cred}; });
        const bool ret = (*reinterpret_cast<Callback*>(cbdata))(creds);
        auto it = creds_ptr;
        for (const auto& [t, p, c, d, res] : creds) {
            it->result =
                static_cast<char*>(malloc(it->resultlen = res.length() + 1u));
...
template <typename Callback>
inline Connection::Connection(gsl::czstring<> name,
...
void Connection::registerCloseCallback(void (*cb)(Data&),
                                       std::unique_ptr<Data> data) {
    virConnectRegisterCloseCallback(
        underlying, reinterpret_cast<virConnectCloseFunc>(cb), &data,
        &std::unique_ptr<Data>::get_deleter());
}
void Connection::registerCloseCallback(void (*cb)()) {
    if (virConnectRegisterCloseCallback(
            underlying, reinterpret_cast<virConnectCloseFunc>(cb), nullptr,
            nullptr))
...
void Connection::unregisterCloseCallback(void (*cb)(Data&)) {
    if (virConnectUnregisterCloseCallback(
            underlying, reinterpret_cast<virConnectCloseFunc>(cb)))
...
template <> auto Connection::listDefinedDomains<gsl::zstring<>>() const {
    ...
    const auto res =
        virConnectListDefinedDomains(underlying, buf.data(), max_size);
    if (res < 0)
    ...
    std::transform(std::make_move_iterator(buf.begin()),
                   std::make_move_iterator(buf.end()), std::back_inserter(ret),
...
auto Connection::listAllDomains(List::Domains::Flags flags) const
    -> std::vector<Domain> {
    std::vector<Domain> ret;
    gsl::owner<virDomainPtr*> domains;

    const auto res =
        virConnectListAllDomains(underlying, &domains, to_integral(flags));
    if (res < 0)
        throw std::runtime_error{"virConnectListAllDomains"};

    ret.reserve(static_cast<unsigned>(res));
    std::for_each(domains, domains + res,
                  [&](virDomainPtr d) { ret.emplace_back(d); });
    std::free(domains);

    return ret;
}

auto Connection::getAllDomainStats(
    Domain::Stats::Types stats, Connection::GetAllDomains::Stats::Flags flags)
    -> std::vector<Domain::Stats::Record> {
    virDomainStatsRecordPtr* ptr;
    const int res = virConnectGetAllDomainStats(underlying, to_integral(stats),
                                                &ptr, to_integral(flags));
    if (res < 0)
    ...
    std::transform(*ptr, *ptr + res, std::back_inserter(recs),
                   [](const virDomainStatsRecord& rec) {
                       return Domain::Stats::Record{rec};
                   });
...
Domain Connection::domainLookupByUUIDString(gsl::czstring<> uuid_str) const
    noexcept {
...
auto Connection::nodeGetCellsFreeMemory() const {
    ...
    const int res = virNodeGetCellsFreeMemory(
        underlying, ret.data(), 0, gsl::narrow_cast<int>(ret.size()));
    if (res < 0)
...
operator|(Connection::List::Domains::Flags lhs,
    ...
    return Connection::List::Domains::Flags(to_integral(lhs) |
                                            to_integral(rhs));
}
...
operator|(Connection::GetAllDomains::Stats::Flags lhs,
    ...
    return Connection::GetAllDomains::Stats::Flags(to_integral(lhs) |
                                                   to_integral(rhs));
}
}