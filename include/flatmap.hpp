...
namespace type_traits {
    template<typename T, typename U> using equal_compare_t = decltype(std::declval<const T &>() ==
                                                                      std::declval<const U &>());

    template<typename T, typename U> using are_equal_comparable = std::experimental::is_detected<equal_compare_t, T, U>;

    template<typename C, typename T, typename U> using is_callable_t = decltype(std::declval<C &>()(std::declval<T>(),
                                                                                                    std::declval<U>()));
...
public:
    ...
    template<typename K, typename V,
            typename = std::enable_if_t<
                    type_traits::is_callable<Compare, Key, K>{} && std::is_constructible<Value, V>{} &&
                    std::is_assignable<Value, V>{}>>
    std::pair<iterator, bool> insert_or_assign(K &&k, V &&v);
...
public:
    ...
    template<typename K, typename V,
            typename = std::enable_if_t<
                    type_traits::is_callable<Compare, K, Key>{} && std::is_constructible<Value, V>{} &&
                    std::is_assignable<Value, V>{}>>
...
class impl::flatmap_storage<Key, Value>::iterator_type {
    template<typename V, typename I> friend
    class impl::flatmap_storage<Key, Value>::iterator_type;
...
public:
    ...
    template<typename C>
    constexpr bool operator==(const iterator_type<C> &ci) const noexcept { return c == ci.c && idx == ci.idx; }

    template<typename C>
    constexpr bool operator!=(const iterator_type<C> &ci) const noexcept { return !(*this == ci); }
...
inline auto unordered_flatmap<Key, Value>::try_emplace(K &&key, V &&... v) -> std::pair<iterator, bool> {
    ...
    this->m_values.emplace_back(std::piecewise_construct, std::forward_as_tuple(std::forward<K>(key)),
